...
def _publish_host_messages(
    publisher, dut: DeviceAdapter, period_ms: int, message_count: int
):
...
def _collect_firmware_messages(messages: queue.Queue, period_ms: int, message_count: int):
    ...
    while len(sequences) < message_count:
        ...
        assert message.startswith(FIRMWARE_MESSAGE_PREFIX), f"unexpected firmware message: {message}"
...
def _run_issue_65_test(dut: DeviceAdapter):
    ...
    try:
        ...
        _wait_for_ready(
            dut, ingress=True, egress=True, period_ms=IDLE_10HZ_PERIOD_MS
        )
        ...
        while time.monotonic() < deadline:
            ...
            message = payload.decode("utf-8")
            assert message.startswith(FIRMWARE_MESSAGE_PREFIX), (
        ...
        assert elapsed >= 2 * 10, f"test ran for only {elapsed:.2f}s"
        assert len(sequences) >= ISSUE_65_MINIMUM_MESSAGES, (
